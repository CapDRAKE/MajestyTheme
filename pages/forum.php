<?php $fofo = $_Forum_->affichageForum(); ?>

<section id="forum" class="welcome-section">
    <div class="container">
        <!-- Recherche -->
		<div class="input-group mx-0 mb-4">
            ...
            <div class="input-group-append">
                <span onclick="searchForum(document.getElementById('search-topic').value, document.getElementById('all-search'), document.getElementById('table-search'),document.getElementById('title-search'))" style="border: none;" class="input-group-text btn btn-primary" style="cursor:pointer;"><i class="fas fa-search"></i>
                </span>
            </div>
        </div>
        <div class="row" style="display:none;" id="all-search">
            <table class="table table-striped shadow-lg" style="display:none;">
                <thead>
                    <tr>
        ...
        <a href="?action=mode_joueur" class="btn btn-primary btn-sm mb-4 float-left">Passer en mode visuel <?= ($_SESSION['mode']) ? "Administrateur" : "Joueur"; ?></a>
        <?php endif; ?>
        ...
        <div class="float-right">
            <div class="dropdown d-inline-block">
                <button class="btn btn-primary btn-sm mb-4 dropdown-toggle" type="button" id="perms<?= $fofo[$i]['id']; ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            ...
            <div class="dropdown d-inline-block">
                <button class="btn btn-primary btn-sm  mb-4 dropdown-toggle" type="button" id="ordreforum<?= $fofo[$i]['id']; ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    Modifier l'ordre
                </button>
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
            ...
            <a href="?action=remove_forum&id=<?php echo $fofo[$i]['id']; ?>" class="btn btn-danger  mb-4 btn-sm">
                <i class="fas fa-trash"></i> Supprimer
            </a>
        </div>
        <?php endif; ?>
        <table class="table table-striped shadow-lg mt-4 mb-4">
            <thead>
                <tr>
                    <th colspan="5" style="width: <?= (Permission::getInstance()->verifPerm('PermsForum', 'general', 'deleteCategorie') and !$_SESSION['mode']) ? '75%' : '100%'; ?>;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="align-middle text-center">
                        ...
                        <?php else : ?>
                        <p class="mb-0"> Il n'y a pas de sujet dans ce forum </p>
                        <?php endif; ?>
                    </td>
                    ...
                    <td class="align-middle">
                        <div class="mx-auto d-flex align-self-center">
                            ...
                            <button type="button" onclick="openModalEditForum(<?= $categorie[$j]['id']; ?>,'<?= $categorie[$j]['nom']; ?>',<?php echo $fofo[$i]['id']; ?>, <?php if ($categorie[$j]['img'] == NULL) { echo 'null'; }  else { echo "'".$categorie[$j]['img']."'";} ?> );" class="btn btn-primary btn-sm ml-1">
                                <i class="fas fa-cog"></i>
                            </button>

                            <?php if ($categorie[$j]['close'] == 0) : ?>

                            <a href="?action=lock_cat&id=<?= $categorie[$j]['id']; ?>&lock=1" title="Fermer le forum" class="btn btn-primary btn-sm ml-1 no-hover">
                                <i class="fas fa-unlock-alt" aria-hidden="true"></i>
                            </a>

                            <?php else : ?>

                            <a href="?action=unlock_cat&id=<?= $categorie[$j]['id']; ?>&lock=0" title="Ouvrir le forum" class="btn btn-primary btn-sm ml-1 no-hover">
                            ...
                            <a href="?action=remove_cat&id=<?= $categorie[$j]['id']; ?>" class="btn btn-danger btn-sm no-hover ml-1">
        ...
        <div class="row px-3 mb-2">
            ...
            <a class="btn btn-primary btn-sm no-hover col-5 mr-auto" role="button" data-toggle="collapse" href="#add_forum" aria-expanded="false" aria-controls="add_forum">
                Ajouter une Catégorie
            </a>
            <?php endif; ?>
            <?php if (Permission::getInstance()->verifPerm('PermsForum', 'general', 'addCategorie') and !$_SESSION['mode']) : ?>

            <a class="btn btn-primary btn-sm no-hover col-5 ml-auto" role="button" data-toggle="collapse" href="#add_categorie" aria-exepanded="false" aria-controls="add_categorie">
                Ajouter un Forum
            </a>
        </div>
	<br/><br/>
            ...
            <div class="card">
                <form action="?action=create_cat" method="post">
                    ...
                    <div class="card-header bg-primary">
                        <h4 class="text-white mb-0">Ajouter un forum</h4>
                    </div>
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary w-100">Créer un Forum</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="collapse" id="add_forum">
            <div class="card">
                <form action="?action=create_forum" method="post">
                    <div class="card-header bg-primary">
                        <h4 class="text-white mb-0">Ajouter une catégorie</h4>
                    </div>
                    <div class="card-body">
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary btn-sm btn-block">Créer une catégorie</button>
                    </div>
